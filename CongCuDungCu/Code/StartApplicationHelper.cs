...
namespace DevExpress.DemoData.Helpers
{
    public static class StartApplicationHelper
    {
        ...
        public static EscapeHelper UrlHelper
        {
            get
            {
                return urlHelper;
        ...
        public static string Start(string fileName, string[] arguments, bool isUrl)
        {
            ...
            var argumentsString = JoinArguments(arguments);
        ...
        private static void StartCore(string fileName, string workingDirectory, string argumentsString)
        {
            ...
            if (StartWithRegistryInfo(fileName, workingDirectory, argumentsString))
            {
                return;
            }
            if (StartWithVSPath(fileName, workingDirectory, argumentsString))
            {
                return;
            }
            DoStart(fileName, workingDirectory, argumentsString);
        ...
        private static bool StartWithRegistryInfo(string fileName, string workingDirectory, string argumentsString)
        {
            try
            {
                var slnApp = Registry.GetValue("HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\FileExts\\.sln\\UserChoice", "ProgId", string.Empty) as string;
                if (string.IsNullOrEmpty(slnApp))
                {
                    var openWithProgids = Registry.CurrentUser.OpenSubKey("Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\FileExts\\.sln\\OpenWithProgids");
                    if (openWithProgids == null)
                    {
                        return false;
                    }
                    if (openWithProgids.ValueCount != 1)
                    {
                        return false;
                    }
                    slnApp = openWithProgids.GetValueNames()[0] as string;
                }
                var vsPath = Registry.GetValue("HKEY_CLASSES_ROOT\\" + slnApp + "\\shell\\Open\\command", null, string.Empty) as string;
                const string vsVersionSelector = "VSLauncher.exe";
                if (vsPath.IndexOf(vsVersionSelector, 0) != (-1))
                {
                    return false;
                }
                ...
                fileName = vsPath.Replace("\"%1\"", string.Empty).Replace("\"", string.Empty);
                DoStart(fileName, workingDirectory, argumentsString);
                return true;
            }
            catch
        ...
        private static bool StartWithVSPath(string fileName, string workingDirectory, string argumentsString)
        {
            try
            {
                var slnFile = new VSSolutionFile(fileName);
                var vsPath = VSInstallHelper.GetVSPath(slnFile.Version);
                if (string.IsNullOrEmpty(vsPath))
                {
                    return false;
                }
        ...
        private class WinApiHelper
        {
            ...
            public static void WaitForWindowAppears(Process process)
            {
                ...
                var handles = new IntPtr[] { process.Handle };
                try
                {
                    while (true)
                    {
            ...
            {
                var path = new StringBuilder(Import.MAX_PATH);
                var result = Import.SHGetFolderPath(IntPtr.Zero, (int)folderType, IntPtr.Zero, 0, path);
                if (result != 0)
                {
        ...
        private class VSInstallHelper
        {
            ...
            private static string GetVSPathCore(string version)
            {
                try
                {
                    var path = RegistryViewer.Current.GetSzValue(RegistryHive.LocalMachine, string.Format("SOFTWARE/Microsoft/VisualStudio/{0}/Setup/VS", version), "EnvironmentPath");
                    if (path == null)
                    {
                        return string.Empty;
                    }
        ...
        private class VSSolutionFile
        {
            ...
            private VSVersion GetVersion()
            {
                ...
                var versionString = text.Select(t => GetVersionInLine(t)).Where(v => v != null).FirstOrDefault();
            ...
            {
                var match = Regex.Match(line, "Microsoft Visual Studio Solution File, Format Version (?<version>.*)");
    ...
    public class EscapeHelper
    {
        ...
        {
            var replace = GetDictionary(replaces, from.Length - 1);
            replace.Add(from, to);
        }
        private static string Replace(string s, List<Dictionary<string, string>> replaces)
        {
            ...
            {
                var originalLength_1 = -1;
                string escaped = null;
                for (var length_1 = replaces.Count - 1; length_1 >= 0; --length_1)
                {
                    if (i + length_1 + 1 > s.Length)
                    {
                        continue;
                    }
                    var dictionary = GetDictionary(replaces, length_1);
                    var t = s.Substring(i, length_1 + 1);
                    var r = (string )null;
    ...
    public class WinApiRegistryMultiKey : DisposableBase
    {
        ...
        {
            var wkey = new WinApiRegistryKey(hive, key, access | WinApiRegistryHelper.ResigtryAccess.WOW64_32Key);
            if (wkey.Exists)
            {
                keys.Add(wkey);
            }
        ...
        public string[] GetMultiSzValue(string name)
        {
            ...
            foreach (WinApiRegistryKey wkey in keys)
            {
                var s = wkey.GetMultiSzValue(name);
                if (s != null)
                {
                    values.AddRange(s);
                }
    ...
    public class RegistryViewer : RegistryViewerBase
    {
        ...
        {
            using (var wkey = GetWKey(hive, key))
            {
                return wkey.Exists;
            }
        }
    ...
    public class DbEngineDetector
    {
        public static string LocalDbVersion
        {
            get
            {
                var keybase = "SOFTWARE\\Microsoft\\Microsoft SQL Server Local DB\\Installed Versions\\";
        ...
        public static bool IsSqlExpressInstalled
        {
            get
            {
                var instances = RegistryViewer.Current.GetMultiSzValue(RegistryHive.LocalMachine, "SOFTWARE\\Microsoft\\Microsoft SQL Server", "InstalledInstances");
        ...
        public static bool IsLocalDbInstalled
        {
            get
            {
                return !string.IsNullOrEmpty(LocalDbVersion);
            }
        }
        public static string[] GetSclCEInstalledVersions()
        {
            var result = new List<string>();
            var keybase = "SOFTWARE\\Microsoft\\Microsoft SQL Server Compact Edition\\";
        ...
        {
            var localdbVersion = LocalDbVersion;
            if (IsSqlExpressInstalled)
            {
                return ".\\SQLEXPRESS";
            }
