...
{
    public partial class rpCacDonViCo : DevExpress.XtraReports.UI.XtraReport
    {
        public rpCacDonViCo()
        {
            InitializeComponent();
        ...
        {
            var db = GetDb();
            var maDonVis = (from p in db.DMDonvis
                            where p.Madvql == IDDonVi
                            select p.Tendonvi);
            DataSet ds = dsCCDC;
            foreach (string maDonVi in maDonVis)
            {
                if(maDonVi.StartsWith("KHO")) continue;
                if (maDonVi.StartsWith("K-PCNT")) continue;
                var tu = maDonVi;
